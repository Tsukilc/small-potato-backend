...
import org.tsukilc.common.util.MessageProducerUtil;
...
import org.tsukilc.interactionservice.util.LikeKeyGenerator;
...
public class LikeServiceImpl implements LikeService {
    ...
    @Autowired
    private MessageProducerUtil messageProducerUtil;
    ...
    public LikeResponse like(LikeRequest request) {
        ...
        if (oldStatus == null) {
            ...
            wrapper.eq(LikeRecord::getUserId, request.getUserId())
                   ...
                   .eq(LikeRecord::getType, request.getType())
                   .last("LIMIT 1");
            
            LikeRecord record = likeRecordMapper.selectOne(wrapper);
            oldStatus = record != null && record.getStatus();
        }

        // 如果操作状态与当前状态相同，则不做任何操作，直接返回
        // 保证接口幂等性
        ...
        // 根据操作类型选择不同的消息标签
        String tag = request.getStatus() ? MQConstant.LIKE_TAG : MQConstant.UNLIKE_TAG;
        
        // 异步发送消息
        messageProducerUtil.sendMessageAsync(MQConstant.LIKE_TOPIC, tag, message);
        ...
        // 如果是笔记类型的点赞，清除用户总点赞数缓存
        if (request.getType() == 1) { // 假设1是笔记类型
    ...
    public Boolean getLikeStatus(Long userId, Long targetId, Integer type) {
        ...
        if (status == null) {
            ...
            LikeRecord record = likeRecordMapper.selectOne(wrapper);
            status = record != null && record.getStatus();
